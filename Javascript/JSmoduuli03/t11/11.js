...
for (let pic of picArray) {
  ...
  img.alt = pic["title"]
  const largeUrl = document.createElement("p")
  largeUrl.innerText = pic["image"]["large"]
  largeUrl.setAttribute("hidden", "true")
  largeUrl.setAttribute("id", "largeURL")
  const caption = document.createElement("figcaption")
  caption.innerText = pic["caption"]
  const figure = document.createElement("figure")
  figure.appendChild(img)
  figure.appendChild(largeUrl)
...
for (let article of articles) {
  article.addEventListener("click", () => {
    const largePic = article.querySelector("#largeURL").innerText
    const title = article.querySelector("img").alt
    dialogImg.src = largePic
    dialogImg.alt = title
    dialog.showModal()
  })
}
